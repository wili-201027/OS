...
USER_CPP := $(shell find $(USER_SRC_ROOT) -type f -name '*.cpp' -not -path '*/compositor/*' -not -path '*/programs/*')
...
# 3. Mapear a objetos
USER_OBJS_CPP := $(patsubst $(USER_SRC_ROOT)/%.cpp,$(USER_OBJ_ROOT)/%.o,$(USER_CPP))
...
USER_OBJS := $(USER_OBJS_CPP) $(USER_OBJS_C) $(USER_OBJS_S) $(USER_OBJS_C_EXTRA)
...
$(USER_OBJ_ROOT)/%.o: $(USER_SRC_ROOT)/%.cpp
	@mkdir -p $(dir $@)
	@echo "[CXX] $< -> $@"
	$(CXX) $(CXXFLAGS) \
		-I$(USER_SRC_ROOT) \
