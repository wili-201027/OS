...
extern void      fb_copy_pixels(const uint32_t *src, uint32_t *dst, uint32_t pixel_count);
...
static uint32_t *s_blur_tmp = NULL;
static uint32_t  s_blur_sz  = 0;

// Double buffering
static uint32_t *s_back_buffer = NULL;
...
static void draw_char_fb(uint32_t*fb,uint32_t fbw,uint32_t fbh,
                          int px,int py,char ch,uint32_t col,int scale)
{
    if (ch >= 'a' && ch <= 'z') ch -= 32;
                    ...
                    int x,int y,const char*s,uint32_t col)
{
 ...
 void draw_string_fb_scaled(uint32_t*fb,uint32_t fbw,uint32_t fbh,
...
{
    if (!ensure_back_buffer(w, h)) return NULL;
