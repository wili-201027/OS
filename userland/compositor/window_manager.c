...
static inline uint32_t rgb(uint8_t r,uint8_t g,uint8_t b){ return RGB(r,g,b); }
...
// DOM API implementations (C exports)
 ...
 void wm_set_element_style(wm_element *el, uint32_t style_flags){ if(!el) return; ((Element*)el)->style_flags = style_flags; }
...
static Element *find_element_by_id_internal(Element *root, const char *id){
 ...
 wm_element* wm_find_element_by_id(void *wv, const char *id){ if(!wv || !id) return NULL; Window *w=(Window*)wv; return (wm_element*)find_element_by_id_internal(w->root,id); }
...
static Element *create_element_internal(const char *tag, const char *id){
    ...
    e->style_flags=0; e->x=e->y=e->w=e->h=0; e->first_child=NULL; e->next_sibling=NULL; e->parent=NULL;
    return e;
}
                                ...
                                int x,int y,int w,int h)
{
    ...
    if ((uint64_t)w * (uint64_t)h > 1024u*1024u) return NULL;
...
// Font 3×5 extern
 void draw_string_fb(uint32_t*,uint32_t,uint32_t,int,int,const char*,uint32_t);
                   ...
                   void(*blur)(uint32_t*,uint32_t,int,int,int,int,int))
{
    ...
    while (s_pending_free) {
        ...
        if (s_taskbar == fw) s_taskbar = NULL;
        if (s_focused == fw) s_focused = NULL;
        if (fw->pixels) { free(fw->pixels); fw->pixels = NULL; }
        free(fw);
    }
}
...
void *wm_create_window(int x,int y,int w,int h,const char*title)
{
    ...
    win->title=title; win->visible=true; win->focused=false; win->minimised=false; win->zombie=false; win->style_flags=0; win->next_free = NULL;
    ...
    win->prev = NULL; win->next = sg_head; if(sg_head) sg_head->prev = win; sg_head = win;
    ...
    if(pressed){
        ...
        if(h){
            ...
            if(in_rs){
                ...
                s_resize=false; s_resize_win=NULL;
            }
        }
        else {
            s_drag=false; s_drag_win=NULL;
...
static void* s_window_ids[MAX_WINDOW_IDS] = {NULL};

static int register_window(void *win) {
    for (int i = 0; i < MAX_WINDOW_IDS; ++i) {
        if (s_window_ids[i] == NULL) {
            s_window_ids[i] = win;
...
static Window* get_window_by_id(int wid) {
    if (wid <= 0 || wid > MAX_WINDOW_IDS) return NULL;
